...
while True:
    t1 = time.time()
    client_socket.sendto(send_size.encode('utf-8'),('127.0.0.1',9802))
    client_socket.settimeout(0.1)
    try:
        data,addr = client_socket.recvfrom(8192)
        rtt = 1000*(time.time() - t1)
        break
    except:
        continue
...
def sending_process() :
    ...
    while True:
        ...
        b = burst_size
        sleep_time = max(b*rtt/1000, 0.02)
        print(b*rtt/1000)
        for j in range(round(b)) :
            ...
            client_socket.sendto(message.encode('utf-8'),('127.0.0.1',9802))
            sent_time_dict[off_sent] = time.time()-start_time
            req_sent += 1
...
def receiving_process() :
    ...
    while True:
        ...
        while time.time()-start<sleep_time:
            ...
            with lock :
                try:
                    time_when_sent = sent_time_dict[offset]
                    rtt_time_list.append(((alpha)*(time.time() - start_time - time_when_sent)*1000 + (1-alpha)*rtt))
                    sample_list.append((time.time() - start_time - time_when_sent)*1000)
                    rtt = max(((alpha)*(time.time() - start_time - time_when_sent)*1000 + (1-alpha)*rtt), 0.1)
                    rtt = min(15.0, rtt)
                except: pass
                    ...
                    # break
            # try : 
            #     rtt = time.time()-sent_time_dict[offset//1448] 
            #     print(rtt)
            # except : 
            #     print(rtt)
...
while True:
    client_socket.sendto(submit.encode('utf-8'),('127.0.0.1',9802))
    client_socket.settimeout(0.1)
    try:
        data,addr = client_socket.recvfrom(8192)
        break
...
plt.figure(figsize=(16,12))
plt.plot(sample_list,linestyle='-',label = "Sample RTT")
plt.plot(rtt_time_list,linestyle='-',label = "Mean RTT",color='red')
plt.xlabel('No of lines received',fontsize = 18)
plt.ylabel('RTT (in ms)', fontsize = 18)
plt.xticks(fontsize = 18)
plt.yticks(fontsize = 18)
plt.title('EWMA',fontsize = 20)
plt.legend()
plt.grid()
plt.savefig("rtt.jpg")
plt.close()
...
plt.figure(figsize=(16,12))
plt.plot(sample_list[:200],linestyle='-',label = "Sample RTT")
plt.plot(rtt_time_list[:200],linestyle='-',label = "Mean RTT",color='red')
plt.xlabel('No of lines received',fontsize = 18)
plt.ylabel('RTT (in ms)', fontsize = 18)
plt.xticks(fontsize = 18)
plt.yticks(fontsize = 18)
plt.title('EWMA (zoomed in)',fontsize = 20)
plt.legend()
plt.grid()
plt.savefig("rtt_zoom.jpg")
plt.close()